...
const TimeSegment = ({ segment, state }: TimeSegmentProps) => {
  ...
  // ✅ Los literales (":" y separadores) NO deben verse como inputs
  if (segment.type === "literal") {
    // Si es un espacio (o separador raro), no renders nada (evita “cajitas vacías”)
    if (!segment.text || segment.text.trim() === "") return null;

    return (
      <span
        aria-hidden="true"
        className="px-1 text-gray-400 tabular-nums select-none"
      >
        {segment.text}
      </span>
    );
  ...
  // ✅ Solo hour/minute (editables) se renderizan como input segmentado
  return (
    <div
      {...segmentProps}
      ref={ref}
      className={cx(
        "relative block w-full appearance-none rounded-md border px-2.5 py-1.5 text-left tabular-nums shadow-sm outline-none transition sm:text-sm",
  ...
  ({ hourCycle, ...props }: TimeInputProps, ref) => {
    ...
    return (
      <div
        {...fieldProps}
        ref={innerRef}
        className="group/time-input inline-flex w-full items-center gap-x-2"
      >
...
const SingleDatePicker = ({
  ...
  const onDateChange = (next: Date | undefined) => {
    if (!next) {
      setDate(undefined);
      return;
    }
    ...
    if (showTimePicker) {
      ...
      nextDate.setHours(t.hour);
      nextDate.setMinutes(t.minute);
    }
...
const RangeDatePicker = ({
  ...
  const onRangeChange = (next: DateRange | undefined) => {
    ...
    if (showTimePicker) {
      if (from) {
        const t = startTime ?? new Time(0, 0);
        from.setHours(t.hour);
        from.setMinutes(t.minute);
      }
      if (to) {
        const t = endTime ?? new Time(0, 0);
        to.setHours(t.hour);
        to.setMinutes(t.minute);
      }
    }
