...
function ViewOptions<TData>({ table }: DataTableViewOptionsProps<TData>) {
  const tableColumns: Item[] = table.getAllLeafColumns().map((column) => {
    const metaLabel =
      typeof column.columnDef.meta?.displayName === "string"
    ...
    return {
      id: column.id,
      label: metaLabel ?? headerLabel ?? column.id,
    };
  });
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button
              variant="secondary"
              className={cx(
                "ml-auto hidden cursor-pointer gap-x-2 px-2 py-1.5 text-sm sm:text-xs lg:flex",
              )}
            >
          ...
          >
            <Label className="font-medium">Mostrar columnas</Label>
